...
int main(int argc, char **argv)
{
    ...
    auto time_1 = chrono::high_resolution_clock::now(); // change to MPI_Wtime()

    // START: the main part of the code that needs to use MPI timing routines 
    ...
    auto time_2 = chrono::high_resolution_clock::now(); // change to MPI_Wtime() 
    ...
    cout << "Elapsed time: "; 
    cout << std::fixed << std::setprecision(4) << chrono::duration<double>(time_2 - time_1).count(); // remove for MPI
    // cout << std::fixed << std::setprecision(4) << (time_2 - time_1); // modify accordingly for MPI
